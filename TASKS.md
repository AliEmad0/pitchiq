...
**⚠️ Still open — the un-anchored pre-2003 tail.** `Sami Hyypiä '99 and '01 rate 95` and `John Arne Riise '01 rates 94`, both **un-anchored**, level with Van Dijk '19 and Schmeichel '92. The amplifier and the boost both land on a statistical base that still runs hot before 2003, and neither is bounded by an anchor. Two candidate fixes, neither attempted here: curate the genuinely great ones (Hyypiä is a reasonable `legend`), or correct the sparse-era scale itself. Riise at 94 is the clearer error and the better test case.
