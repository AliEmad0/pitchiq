...
**Prerender `/teams/[id]`, `/managers/[id]` + the dashboard — drop the server `?season=` read** · 🚧 In progress (only M71b remains, blocked) · `P2` · `L` · Type: Perf + UI

> **⭐ START HERE — state as of 2026-07-30.** Decomposed into three sub-projects. **M71a ([PR #64](https://github.com/AliEmad0/pitchiq/pull/64)) and M71c ([PR #70](https://github.com/AliEmad0/pitchiq/pull/70)) are SHIPPED and production-verified** — every entity DETAIL route (`/`, `/players/[id]`, `/fixtures/[id]`, `/teams/[id]`, `/managers/[id]`) and the season pages are now prerendered + CDN-served, and the cache guard enforces all of them. **The only remaining sub-project is M71b (the section INDEX pages), which is blocked on an owner design pass** — see its row below.
>
> |          | Scope                                                                                                         | State                                                                                                                                                                                                                                                                                                        |
> | -------- | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
...
> | **M71b** | Section indexes (`/teams`, `/players`, `/fixtures`, `/leaderboards`, `/managers`) under the namespace         | **Blocked on an owner design pass** — the URL model (`/seasons/<year>/teams` vs client-side swap on `/teams`) was scoped out of the M71a spec and is not settled; don't start it without that decision                                                                                                       |
