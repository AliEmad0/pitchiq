...
**Pitch-view refinements — owner feedback 2026-08-03 (implement NEXT session, on top of PR #85):**

- [ ] **1. Horizontal pitch.** Rotate `MatchPitch` to landscape / broadcast (home left half, away right; map formation `row` → x, `col` → y). Currently portrait (`viewBox 0 0 100 140`, home bottom / away top).
- [ ] **2. Jersey numbers + short names.** Render a squad number inside each player dot + a short surname beneath it. Needs per-slot `{name, number}` threaded through `MatchViewModel` (the view-model currently passes only `slots {row,col,role}`). `GamePlayer` has `name` but **no shirt number** → derive 1–11 by slot for v1, or source real numbers from `lineups-*`.
- [ ] **3. Dynamic player movement.** Subtle transform/opacity pulse on active players during key events (not static dots). New keyframe in `globals.css` (transform IS allowlisted), reduce-gated. Owner to decide: its own mini 30+30 animation pass, or a direct refinement?
- [ ] **4. Win-prob bar out of the pitch canvas.** It currently overlays + obscures the bottom defence/GK. Move it **below the scoreboard, outside the pitch `<div>`**. Fix the "Draw" label to sit over its grey segment, and the cramped `0%0%` when draw/away segments are tiny (hide the % label under ~12% width).
- [ ] **5. Commentary timing + feed.** High-importance events (goal/card) get a **minimum ~2.5s dwell** before advancing (event-paced queue, not the fixed 280 ms/min tick); add a short scrollable recent-history feed.
- [ ] **6. Playback speed controls.** Add a `1x / 2x / 4x` toggle alongside Play/Pause/Restart (multiply `TICK_MS`).
- [ ] Minor: investigate the faint ghost row under the scoreboard (z-index/overlay artifact seen in the owner screenshot); consider a real club-abbreviation map (e.g. `MAN` → `MUN`).
