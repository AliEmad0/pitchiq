...
- ⚠️ **A reduced-motion defect the live screen surfaced.** `minute` is seeded from the ceiling once and the clock effect returns early when `reduced`, so the screen stuck at whatever minute the FIRST decision landed on and never showed the rest of the match. Measured: without the effect that follows the ceiling, the live suite fails **5 of 10 runs**. ⚠️ **`MatchView` has the same shape and is NOT fixed** — it is the shipped screen for the other packs and changing it risks their tests, so it is left as a separate ticket.
