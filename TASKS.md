...
| [TASK-1844](#task-1844) | Calibrate the engine's sensitivity to squad quality             | 📋 Todo    | P2       | M   |
...
**Owner review, 2026-08-26 — prices rescaled and both surfaces redesigned.**

- ⛔ **The prices were unusable.** A **£104M card against a £100M budget**, reported by the owner. Card prices are now a **GAME currency in the FPL idiom** — `FLOOR 4.0m + (19.5m − 4.0m) × percentileInPool ^ 2.0`, in integer tenths of a million (`domain/price-band.ts`). Kompany 2011 goes €103.9M → **£16.4m**, Haaland tops out at **£19.5m**, and top players stay under £20m as the owner asked.
- ⭐ **Compression is safe because a bargain is a RANK DISAGREEMENT between rating and price**, and the mapping is strictly monotonic in the indexed value — so every disagreement survives. Vardy 2015 is 91-rated at **£8.5m** against Rodri 2020, 91-rated at **£15.1m**.
- ⛔ **By PERCENTILE, never a linear or log rescale.** Market values are heavily skewed, so a linear map crushes ~90% of the pool into £4.0–7.4m.
- ⛔ **Compression nearly destroyed the balance, and only measuring caught it.** Squeezing a 40× range into 4.9× makes quality almost free — at the first attempt £100m bought **97% of the best possible XI**. Deepening the pool was tried and rejected (1,800 cards, rating floor 53, moved it only to 95.2% at triple the payload). The **curve** is the lever: at 2.0 a median-pick XI costs £81m and the priciest-pick XI £150m, so the cap binds in every room while leaving ~£19m to spend up.
- ⭐ **The rescale changed what `DealOptions.cheapest` is FOR, and the test was rewritten to say so.** The worst naive floor is now **£65.0m against a £100.0m cap** — feasible everywhere — so it is no longer a feasibility fix but a HEADROOM one, dropping the floor to £44.3m. ⚠️ A control that stops failing is a control that has stopped proving anything; assert what is true now, do not delete the trail.
- ⚠️ **£, not €** — the app shows real market values in € elsewhere, so a different mark stops a derived game price being read as one.
- ⭐ **Both surfaces redesigned from 30-concept galleries** (owner picked, 2026-08-26): the card price is **"Dark slab"** — a solid black plate at TOP CENTRE, 15px, opaque because that is exactly where the photo's head sits — and the meter is **"Countdown"**, the wallet as the hero number with the ceiling as a bar beneath. The first meter gave all three numbers equal weight, so the one that gates the cards was the hardest to find.
- **Re-verified in the browser**: 11 rounds greedily-expensive, **£99.7m of £100.0m**, never a dead hand, and 4 of 5 blocked in the closing rounds.

**Owner review round 2, 2026-08-26 — a SIXTEEN-man squad, editable picks, and a legible meter.**

- ⛔ **The meter's bar was misread on sight, which means it was wrong.** It showed the CEILING as a share of the budget, so it began near 60% and read as "half my money is already gone" before a single pick. It now tracks **spend** (empty at the start) and a second line says **why** the spendable figure is below the balance — "£40.0m held back for the other 10 positions". The numbers were always right; the encoding lied.
- ⭐ **The meter moved ABOVE the pitch** (owner). Under it, the one figure every decision depends on sat below the fold on a laptop.
- ⭐ **Picks are no longer final, and the coach confirms when ready.** `DraftSpec` gains `lockPicks: false` + `confirm`, so re-opening a filled slot re-offers its five and a swap REPLACES rather than adds. Verified: £4.0m → £14.3m left £105.7m, not £101.7m.
- ⭐ **£120m now buys SIXTEEN players** — the XI plus a five-man bench the coach drafts and pays for. "One must be a keeper" is true by construction: `BENCH_SHAPE` opens with a GK. Measured over 60 rooms: a squad costs **£64m** at the floor, **£114m** at median picks and **£211m** taking the dearest each round, so £120m binds in 100% of rooms and is never infeasible — but leaves only **+£6m** against a median squad, deliberately tighter than the £19m the XI-only £100m gave. **£130m would restore the old feel.**
- ⛔ **A paid bench must be the bench that PLAYS, and it must survive resume and share.** `buildSession` splits the squad at `formation.slots.length` and is the **only** place that knows where the XI ends — so storage, the share code and both replays keep carrying one ordered list and needed no concept of a bench. The share codec's card block was always `_`-delimited; only two checks pinned it to eleven, now a bounded 11–16 (bounded, because a code is untrusted input).
- ⛔ **The rival pays for its bench too.** Capping only its XI would buy the coach sixteen players and the rival eleven for the same money — the 2026-08-19 balance defect in a new hat, invisible on screen.
- ⛔ **A browser found a crash no test did, again.** Opening the first BENCH slot threw `Cannot read properties of undefined (reading 'role')`: the veil read `shape.slots[veil.slot].role`, and a bench index is past the end of an eleven-slot formation. Every role lookup now goes through `roleAt`, which falls through to the bench list.
- ⚠️ **Van der Sar 94 at £4.6m is CORRECT, not a bug** (owner query). His real indexed value is €18.1M against Pickford's €19.6M — he was 37 that season, so the market priced him like a squad keeper while his performance rates 94. That gap IS the bargain. Checked it does not trivialise the mode: the cheapest all-88+ XI is **£107.5m** against the cap, and only 2 of 53 cards at 88+ are under £6m. ⚠️ And the "veterans are systematically cheap" hypothesis is **wrong** — mean age is 26.0 in every price band.

**Owner review round 3, 2026-08-26 — the swap was only half built, and the missing half LOCKED THE PAGE.**

- ⛔ **THE BUG: a full squad could not be changed, and could not leave the screen.** With the sixteen filled and £1.2m of change left, tapping a filled position dealt a hand in which **every card was disabled** — and a round has no close control, no backdrop dismiss and no Escape. The page was stuck until a reload threw the whole draft away. Two independent defects compounding:
  1. `budgetView` counted **every** filled slot into `spent`, the OPEN one included, because the `picked != null` branch `continue`d before the `i === open` check was ever reached. So the ceiling on a swap was the loose change alone rather than `change + the fee of the man being replaced`.
  2. A `round` veil is deliberately non-dismissable — that is Legacy Club's rule, and a good one, because there the round IS the commitment. Budget Cap is the only pack with `lockPicks: false`, and it re-used that same round for a **voluntary** re-open the coach initiated and must be able to abandon.
- ⭐ **The fix is one idea stated once: the OPEN SLOT CONTRIBUTES NOTHING** — not its hand's minimum to the reserve (which always worked), and not its occupant's fee to the spend (which never did). ⭐ That also **restores the mode's central guarantee** rather than patching around it: the state before a re-open was valid, so `cost(occupant) ≤ ceiling` follows directly, and since he was dealt from THAT hand the hand can never be entirely dead. Spec §5.4 now carries properties 3 and 4 — its property 2 ("no hand is ever dead") had only ever been proved for an EMPTY open slot.
- ⭐ **Tapping the man you already own DROPS him** (owner's words: "clicking his card a second time deselects the player"). New `clear` action on `roomReducer`; it refuses the LOCKED slot, because Captain's Draft's icon is not a pick the coach made. Under a budget the real move is "I cannot afford anyone here yet — take him off and go spend it at the back", and a hand that can only ever REPLACE forces the coach to buy someone he does not want in order to sell.
- ⭐ **Three ways out, and a heading that admits what it is.** A "Keep {name}" button, the backdrop and Escape, all gated on `lockPicks !== true` so Legacy and Captain's Draft are untouched; the man you hold is ringed and marked "In your squad"; the heading reads **"Change your CF"**, not "Choose your CF", over a filled slot; and the hint names the money — "Dropping him puts £14.2m back in the budget."
- ⛔ **The fixture said the OPPOSITE of the shipped pack, which is why 126 tests missed it.** `budget-draft.test.tsx` declared `lockPicks: true` while `BUDGET_PACK` ships `false` — the one field that decides what tapping a filled slot does — so every test in the file exercised the read-only path of a mode that has no read-only path. It now reads the value **off the pack**. Third instance of the rule: **a fixture that contradicts the thing it stands for proves nothing.**
- ⭐ **The bench is a SHELF now, not five loose boxes** (owner: "hard to see them at first glance"). It shipped as five 92px tiles reading "GK 74" under a full-bleed pitch — neither who the man was nor what he cost, the two things a squad list is for when money is the game. It has a frame, a "Bench · 3 of 5" count, and per tile the role, the **pitch's own gold/silver disc**, the NAME and the FEE; an empty tile is dashed and reads "+ Add GK". Reusing the pitch's disc is the point — the bench is five of the sixteen, not a lesser second list.
- ✅ **£120m STAYS** (owner, 2026-08-26). The £130m alternative flagged in round 2 is closed, not pending.
- ⛔ **Follow-up: #199 also rewrote two SHARED strings in place, so Legacy Club and Captain's Draft had been lying for a week.** `pitchRoundHint` and `pitchNoTimer` were re-pointed at Budget Cap — _"you can change your mind until you confirm"_ and _"swap anyone out until the squad and the money both work"_ — while `PitchDraft` renders both on **every** pack's round. Those two packs ship `lockPicks: true` and no `confirm`, so the screen promised a confirm button it does not have, a swap it does not offer, and money that mode has none of; the strings it overwrote had been correct for Legacy for months. ⭐ Each sentence is now keyed on **the pack field that makes it true** — finality on `lockPicks`, the 80+ promise on `standout` — restoring the pre-#199 wording for locked packs. ⚠️ **Deliberately NOT one combined string**: all three shipped packs have `lockPicks === standout`, so folding them would be accidentally correct today and a lie the day a pack ships `lockPicks: true` with no standout, which is a legal spec. A control test pins exactly that, and all three sabotages were verified to fail.
- **Verified in a real browser, both locales**: drafted sixteen down to **£0.1m remaining** — tighter than the reported case — re-opened a slot and all five cards were buyable where all five had been dead; drop / keep / Escape / backdrop all correct; confirm → matchday programme; `/ar` renders every new string in Arabic; and Legacy Club still refuses to dismiss its round. A **Playwright guard** now pins the whole interaction on the real surface.
...
**⛔ BLOCKED on [TASK-1844](#task-1844) — the engine cannot carry a season yet (2026-09-01).**
Designing this measured the match engine against six real seasons played by their real squads:
it is **under-dispersed by about half** (points SD 8.7 against a real 16.2; top-to-bottom 32.7
against 62.0), because `goalChance` derives its edge from a bounded ratio that barely responds
to a rating gap. Over 90 minutes that reads as drama; over 38 weeks it means the table is mostly
noise and the coach's squad does not decide his position. Full numbers and the one-line fix are
in TASK-1844.
...
**Calibrate the engine's sensitivity to squad quality** · 📋 Todo · `P2` · `M` · Type: Fix
...
- ⚠️ **A unit test cannot prove the matcher routes those paths to middleware** — `probe-path.test.ts` pins the predicate (with a drift guard read off the real route tree), and `cache-guard.yml` gains `scanner` + `branded404` probes against production, where a **tiny body** (0 B middleware / ~59 B platform-firewall 403) is the fingerprint of a function-free answer vs the ~100 KB branded page — the guard accepts either denying layer, because which shapes Vercel's own firewall covers can change without notice.
