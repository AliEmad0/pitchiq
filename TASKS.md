...
- ⛔ **Found while verifying, NOT fixed — a share link still does not replay in `next dev`.** Following a freshly-copied Legacy link lands on the hub: the token stream diverges from the live match at the SECOND decision. Proven not to be either change above — a full live → `encodeTokens` → `replayWith` round trip over the real Liverpool pool reproduces byte-for-byte, with and without the opponent policy, and the page's pool is byte-identical across loads (873 cards, same hash). So the live drive and the replay drive are building different matches for a reason that is upstream of both fixes. Needs its own ticket.
