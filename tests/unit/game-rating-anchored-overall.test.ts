...
/**
 * TASK-1821 Layer 2 wiring — an anchored player's `overall` becomes their heritage
 * anchor plus a bounded season delta; an un-anchored player is untouched and stays on
 * the statistical model (Layer 3 will give those a clamped role amplifier).
 *
...
const ANCHORED = 1003185; // Alan Shearer — anchor 88 in 1994
const ANCHORED_PEER = 1002939; // anchor 85 in 1994
...
describe("rate — Layer 2 anchoring", () => {
  it("puts every anchored player inside ±6 of their heritage anchor", () => {
    ...
    // poor season is the discriminating case — raw it rates 16 against an anchor of 85.
  ...
  it("leaves an un-anchored player exactly on the statistical model", () => {
    expect(rate(cohort[2], ctx).ratings.overall).toBe(rateOutfield(cohort[2], ctx).overall);
  });
  ...
  it("shrinks the delta for an anchored cameo season", () => {
    ...
    expect(Math.abs(short - 88)).toBeLessThan(2);
  });
});
